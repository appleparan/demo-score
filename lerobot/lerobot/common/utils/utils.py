...
def init_hydra_config(config_path: str, overrides: list[str] | None = None, caller_path=None) -> DictConfig:
    ...
    init_path = _relative_path_between(Path(config_path).absolute().parent, Path(__file__).absolute().parent)
    hydra.initialize(
        str(init_path),
        version_base="1.2",
    )
