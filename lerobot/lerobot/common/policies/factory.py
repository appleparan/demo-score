...
def make_policy(
    ...
    if getattr(hydra_cfg, "return_conditioned", False):
        print("Using Return-Conditioned Policy")
        return_conditioned = True
        policy_cfg.input_shapes['observation.state'][0] = policy_cfg.input_shapes['observation.state'][0] + 1

        #Manually set normalization stats for return-condition observation so that normalization has no effect
        dataset_stats['observation.state']['min'] = torch.cat((dataset_stats['observation.state']['min'], torch.tensor([0.])))
        dataset_stats['observation.state']['max'] = torch.cat((dataset_stats['observation.state']['max'], torch.tensor([1.])))
        dataset_stats['observation.state']['mean'] = torch.cat((dataset_stats['observation.state']['mean'], torch.tensor([0.])))
        dataset_stats['observation.state']['std'] = torch.cat((dataset_stats['observation.state']['std'], torch.tensor([1.])))
    else:
        print("Normal, non-Return-Condi")
