...
def load_hf_dataset(repo_id: str, version: str, root: Path, split: str) -> datasets.Dataset:
    ...
    if root is not None:
        ...
        if split != "train":
            ...
            match_from_to = re.search(r"train\[(\d+):(\d+)\]", split)
            match_from = re.search(r"train\[(\d+):\]", split)
            match_to = re.search(r"train\[:(\d+)\]", split)
            if match_from_to:
                from_frame_index = int(match_from_to.group(1))
                to_frame_index = int(match_from_to.group(2))
                hf_dataset = hf_dataset.select(range(from_frame_index, to_frame_index))
            elif match_from:
                ...
                raise ValueError(
                    f'`split` ({split}) should either be "train", "train[INT:]", or "train[:INT]", or "train[INT:INT]"'
                )
    else:
...
def reset_episode_index(hf_dataset: datasets.Dataset) -> datasets.Dataset:
    ...
    needs_mod = False
    for k, v in episode_idx_to_reset_idx_mapping.items():
        #print(k, v, needs_mod)
        if int(k) != int(v):
            needs_mod = True
            break

    #HACK if only one episode we just override episode_index in __getitem__
    if len(episode_idx_to_reset_idx_mapping) == 1:
        needs_mod = False

    if needs_mod:
        #NOTE: Alec: Default in codebase is .map(). Using .with_transform() could make it slower or cause bug but involves less precomputation
        if True:
            def modify_ep_idx_func(example):
                example["episode_index"] = episode_idx_to_reset_idx_mapping[example["episode_index"].item()]
                return example
            hf_dataset = hf_dataset.map(modify_ep_idx_func, keep_in_memory=True, num_proc=os.cpu_count())
        else:
            #TODO: currently not working
            format = hf_dataset.format.copy()
            def modify_ep_idx_func(example):
                print(example['episode_index'], episode_idx_to_reset_idx_mapping)
                example['episode_index'] = [episode_idx_to_reset_idx_mapping[int(example['episode_index'][i])] for i in range(len(example['episode_index']))]
                return example
            hf_dataset = hf_dataset.with_transform(modify_ep_idx_func)
            hf_dataset.set_format(**format)
    else:
        pass
        #print("No need to modify dataset")
...
def make_dataset_from_datasets_file(datasets_file):
