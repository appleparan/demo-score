...
def cfg_to_group(cfg: DictConfig, return_list: bool = False) -> list[str] | str:
    ...
    if isinstance(cfg.dataset_repo_id, str) or isinstance(cfg.dataset_repo_id, Path):
        dataset_name = cfg.dataset_repo_id
    else:
        dataset_name = "_".join(cfg.dataset_repo_id)

    if len(dataset_name) > 54:
        dataset_name = dataset_name[:54]
