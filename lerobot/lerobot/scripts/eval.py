...
def rollout(
    ...
    while not np.all(done):
        ...
        #add return conditioning input to state vector part of observation here
        #should be here so not included in observation that gets stored when doing rollouts
        if return_conditioned:
            observation['observation.state'] = torch.cat((observation['observation.state'], torch.ones((observation['observation.state'].shape[0], 1), device=observation['observation.state'].device)), dim=1)
