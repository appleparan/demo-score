...
def train(cfg: DictConfig, out_dir: str | None = None, job_name: str | None = None):
    ...
    pretrain_path = str(logger.last_pretrained_model_dir) if cfg.resume else None
    loaded_weights = False
    if getattr(cfg, "load_weights", False) is not False:
        pretrain_path = str(cfg.load_weights)
        loaded_weights = True


    policy = make_policy(
        hydra_cfg=cfg,
        dataset_stats=offline_dataset.stats if not (cfg.resume or loaded_weights) else None,
        pretrained_policy_name_or_path=pretrain_path,
    )
    ...
    def evaluate_and_checkpoint_if_needed(step):
        ...
        if cfg.training.eval_freq > 0 and step % cfg.training.eval_freq == 0 or (step < cfg.training.eval_freq*1.8 and step % int(cfg.training.eval_freq / 5) == 0):
        ...
        if cfg.training.save_checkpoint and (
            ...
            or (step < cfg.training.save_freq*1.8 and step % int(cfg.training.save_freq / 5) == 0)
        ):
