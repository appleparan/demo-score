...
def get_ee_pos(env, env_id, side='right'):
    if side == 'right':
        return (env.envs[env_id].env.env._env._physics.named.data.xpos['vx300s_right/left_finger_link'] +\
                env.envs[env_id].env.env._env._physics.named.data.xpos['vx300s_right/right_finger_link'])/2
    else:
        return (env.envs[env_id].env.env._env._physics.named.data.xpos['vx300s_left/left_finger_link'] +\
                env.envs[env_id].env.env._env._physics.named.data.xpos['vx300s_left/right_finger_link'])/2
...
def rollout(
    ...
    while not np.all(done):
        ...
        for env_id in range(len(env.envs)):
            if done[env_id]:
                ee_pos_right[env_id].append(np.array([np.nan, np.nan, np.nan]))
                ee_pos_left[env_id].append(np.array([np.nan, np.nan, np.nan]))
            else:
                ee_pos_right[env_id].append(get_ee_pos(env, env_id, 'right'))
                ee_pos_left[env_id].append(get_ee_pos(env, env_id, 'left'))
    ...
    for env_id in range(len(env.envs)):
        ee_pos_right[env_id] = np.vstack(ee_pos_right[env_id])
        ee_pos_left[env_id] = np.vstack(ee_pos_left[env_id])

    ret['ee_pos_right'] = ee_pos_right
    ret['ee_pos_left'] = ee_pos_left
    return ret
...
def eval_policy(
    ...
    for batch_ix in progbar:
        ...
        ee_pos_right.extend(rollout_data['ee_pos_right'])
        ee_pos_left.extend(rollout_data['ee_pos_left'])
    ...
    info['ee_pos_right'] = ee_pos_right
    info['ee_pos_left'] = ee_pos_left
...
def main(
    ...
    ee_pos_right = np.stack(info['ee_pos_right'], axis=0)
    np.save(Path(out_dir) / "ee_pos_right.npy", ee_pos_right)

    ee_pos_left = np.stack(info['ee_pos_left'], axis=0)
    np.save(Path(out_dir) / "ee_pos_left.npy", ee_pos_left)


    del info['ee_pos_right']
    del info['ee_pos_left']
    ...
    return ee_pos_right, ee_pos_left
