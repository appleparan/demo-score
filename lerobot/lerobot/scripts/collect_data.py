...
def collect_policy_rollouts(
    ...
    info["fps"] = env.unwrapped.metadata["render_fps"]
...
def _compile_episode_data(
    ...
    #NOTE breaks saving but would need this if want to use as online buffer.
...
def save_dataset(directory, hf_dataset, episode_data_index, fps):
    ...
    hf_dataset.with_format(None).save_to_disk(str(directory / "train"))
    hf_dataset.set_transform(hf_transform_to_torch)

    lr_dataset = LeRobotDataset.from_preloaded(hf_dataset=hf_dataset, episode_data_index=episode_data_index)
    stats = compute_stats(lr_dataset)
...
def main(
    ...
    save_dataset(out_dir, fps=info['fps'], **info['episodes'])
